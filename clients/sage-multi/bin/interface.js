...
program.description('Evaluates a set of SPARQL queries using the web preemption model')
    ...
    .option('-m, --method <method>', 'Evaluates the SPARQL query using the specified method: [mono, multi, alpha]', 'multi')
    ...
    return new Promise((resolve, _) => {
        ...
        get_client(server, graph, spy).execute(query, program.timeout).subscribe(
            (solution) => {
                ...
                let statistics = [execution_time, http_calls, data_transfer, nb_results, 'error']
                ...
                let statistics = [execution_time, http_calls, data_transfer, nb_results, state]
                resolve([solutions, statistics])
            }
        )
    })
...
async function execute(server, graph, query) {
    ...
    - solutions: ${solutions.length} solution mappings`)
}
